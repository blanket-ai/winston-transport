...
TransportStream.prototype._write = function _write(info, enc, callback) {
  ...
  if (!level || this.levels[level] >= this.levels[info[LEVEL]]) {
    ...
    var errState;
    var transformed; // We trap(and re-throw) any errors generated by the user-provided format, but also
    // guarantee that the streams callback is invoked so that we can continue flowing.

    try {
      transformed = this.format.transform(_.cloneDeep(info), this.format.options);
    } catch (err) {
      errState = err;
    }
